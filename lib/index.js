...
function PDFInvoice({
  ...
  return {
    genHeader(){
      ...
      .fontSize(11)
      .font('Helvetica-Bold')
      .text('TO:', CONTENT_LEFT_PADDING, 180);
      doc
      .font('Helvetica')
      .text(customer.name || '');
    ...
    genCustomerInfos(){
      doc
      .font('Helvetica-Bold')
      .text('FROM:', CONTENT_LEFT_PADDING * 5, 180);
      doc
      .font('Helvetica')
      .text(company.name || '');
    ...
    genTableHeaders(){
      [
        ...
        const n = (text === 'createdAt' || text === 'price' || text === 'amount') ? 130 : 0;
        const k = (text === 'amount') ? 25 : 0;

        doc
          .fontSize(TEXT_SIZE)
          .font('Helvetica')
          .text(translate[text], table.x + n + k + i * table.inc, table.y);
      });
    },
      ...
      items
        ...
        .forEach((item, itemIndex) => {
          [
            ...
            const n = (field === 'createdAt' || field === 'price' || field === 'amount') ? 130 : 0;
            const k = (field === 'amount') ? 25 : 0;

            doc
              .fontSize(TEXT_SIZE)
              .text(item[field], table.x + n + k + i * table.inc, table.y + TEXT_SIZE + 6 + itemIndex * 15);
          });
        });
    },

    genTotal(){
      let message = `TOTAL ${invoiceCurrency} net without VAT ${invoiceCurrency} ${charge.amount}`;
      if (customer.vat) {
        const VAT = ~~customer.vat;
        const vatAmount = (charge.amount/100 * VAT).toFixed(2);
        const total = (charge.amount + (charge.amount/100 * VAT)).toFixed(2);
        message = `TOTAL ${invoiceCurrency} net ${charge.amount}       VAT ${customer.vat} % ${invoiceCurrency} ${vatAmount}       Total ${invoiceCurrency} ${total}`;
      }
      doc
      .font('Helvetica')
      .fontSize(12)
      .text(`${message}`, CONTENT_LEFT_PADDING, doc.y + 20);
    },
